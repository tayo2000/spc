...
(function($) {
	function getRowIndex(target, value) {
		...
		return $.easyui.indexOfArray(state.data, state.options.valueField,
				value);
	}
	...
	function loadData(target, data, remainText) {
		...
		opts.view.render.call(opts.view, target, $(target).combo('panel'),
				state.data);
	...
	function doQuery(target, q) {
		...
		if (opts.mode == 'remote') {
			...
			var panel = $(target).combo('panel');
			panel.find('.combobox-item-hover')
	...
	function scrollHandler(e) {
		...
		if (opts.groupPosition == 'sticky') {
			...
			$(this).children('.combobox-group:visible').each(function() {
				...
				var rowData = state.data[groupData.startIndex + groupData.count
						- 1];
	...
	$.fn.combobox.parseOptions = function(target) {
		...
		return $.extend({}, $.fn.combo.parseOptions(target), $.parser
						.parseOptions(target, ['valueField', 'textField',
										'groupField', 'groupPosition', 'mode',
										'method', 'url', {
		...
		render : function(target, container, data) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				var cls = 'combobox-item'
						...
						+ (g ? ' combobox-gitem' : '');
				dd.push('<div id="' + (state.itemIdPrefix + '_' + i)
						+ '" class="' + cls + '">');
	...
	$.fn.combobox.defaults = $.extend({}, $.fn.combo.defaults, {
		...
		finder : {
			...
			getGroupEl : function(target, gvalue) {
				var state = $.data(target, 'combobox');
				var index = $.easyui
						.indexOfArray(state.groups, 'value', gvalue);
			...
			getRow : function(target, p) {
				var state = $.data(target, 'combobox');
				var index = (p instanceof $) ? p.attr('id')
