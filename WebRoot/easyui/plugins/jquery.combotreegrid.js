	...
	function _1(_2) {
		...
		_3.grid.treegrid($.extend({}, _4, {
					...
					onClickRow : function(_e) {
						if (_4.multiple) {
							$(this).treegrid(
									_e.checked ? "uncheckNode" : "checkNode",
									_e[_4.idField]);
			...
			if (q) {
				...
				$.map(qq, function(q) {
					...
					if (q) {
						...
						if (v == undefined) {
							$.easyui.forEach(_25.mappingRows, false, function(
									row) {
								if (q.toLowerCase() == String(row[_25.treeField])) {
									v = row[_25.idField];
									return false;
	...
	$.fn.combotreegrid.defaults = $.extend({}, $.fn.combo.defaults,
			$.fn.treegrid.defaults, {
				...
				filter : function(q, row) {
					...
					return (row[_37.treeField] || "").toLowerCase().indexOf(q
							.toLowerCase()) >= 0;
				}
			});
})(jQuery);
