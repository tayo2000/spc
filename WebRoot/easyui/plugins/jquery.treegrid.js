...
(function($) {
	...
	function _42(_43, _44) {
		...
		if (_45.view.hasCheckbox(_43, row)) {
			if (!ck.length) {
				...
				$("<span class=\"tree-checkbox\"></span>").insertBefore(tr
						.find(".tree-title"));
			}
	...
	function _47(_48, _49) {
		...
		var _4b = $(_48).datagrid("getColumnFields", true).length
				+ (_4a.rownumbers ? 1 : 0);
		...
		function _4d(tr, _4e) {
			$("<tr class=\"treegrid-tr-tree\">"
					+ "<td style=\"border:0px\" colspan=\"" + _4e + "\">"
	...
	function _4f(_50, _51, _52, _53, _54) {
		...
		if (_57) {
			...
			var cc1 = _58.next("tr.treegrid-tr-tree").children("td")
					.children("div");
			var cc2 = _59.next("tr.treegrid-tr-tree").children("td")
					.children("div");
			if (!_53) {
				_57.children = [];
			}
	...
	function _b4(_b5) {
		...
		function _b9(_bb) {
			...
			if (_bb.length) {
				...
				_be.find(".tree-join").removeClass("tree-join")
	...
	$.fn.treegrid.methods = {
		...
		reloadFooter : function(jq, _d0) {
			return jq.each(function() {
						...
						if (_d1.showFooter) {
							_d1.view.renderFooter.call(_d1.view, this,
									dc.footer1, true);
							_d1.view.renderFooter.call(_d1.view, this,
									dc.footer2, false);
		...
		clearChecked : function(jq) {
			return jq.each(function() {
						...
						$.map($(_db).treegrid("getCheckedNodes"),
								function(row) {
	...
	$.fn.treegrid.parseOptions = function(_dd) {
		return $.extend({}, $.fn.datagrid.parseOptions(_dd), $.parser
						.parseOptions(_dd, ["treeField", {
											checkbox : "boolean",
	...
	var _de = $.extend({}, $.fn.datagrid.defaults.view, {
		render : function(_df, _e0, _e1) {
			...
			function _e7(_e8, _e9, _ea) {
				...
				var _ec = (_eb ? _eb.children.length : $(_df)
				...
				for (var i = 0; i < _ea.length; i++) {
					...
					var _ef = _e4 + "-" + (_e8 ? 1 : 2) + "-"
							+ row[_e2.idField];
					_ed.push("<tr id=\"" + _ef + "\" node-id=\""
							+ row[_e2.idField] + "\" " + cls + " " + _ee + ">");
					_ed = _ed.concat(_e5.renderRow.call(_e5, _df, _e3, _e8,
							_e9, row));
					_ed.push("</tr>");
		...
		renderFooter : function(_f0, _f1, _f2) {
			...
			for (var i = 0; i < _f4.length; i++) {
				...
				_f6.push("<tr class=\"datagrid-row\" node-id=\""
						+ row[_f3.idField] + "\">");
		...
		renderRow : function(_f7, _f8, _f9, _fa, row) {
			...
			for (var i = 0; i < _f8.length; i++) {
				...
				if (col) {
					var css = col.styler
							? (col.styler(row[_fd], row) || "")
							: "";
					var cs = this.getStyleValue(css);
					var cls = cs.c ? "class=\"" + cs.c + "\"" : "";
					var _fe = col.hidden ? "style=\"display:none;" + cs.s
							+ "\"" : (cs.s ? "style=\"" + cs.s + "\"" : "");
					...
					if (col.checkbox) {
						...
						cc.push(" name=\"" + _fd + "\" value=\""
								+ (row[_fd] != undefined ? row[_fd] : "")
								+ "\">");
					} else {
						var val = null;
						if (col.formatter) {
						...
						if (_fd == _fc.treeField) {
							...
							if (this.hasCheckbox(_f7, row)) {
								var _ff = 0;
								var crow = $.easyui.getArrayItem(
										_fb.checkedRows, _fc.idField,
										row[_fc.idField]);
								if (crow) {
									_ff = crow.checkState == "checked" ? 1 : 2;
								} else {
									var prow = $.easyui.getArrayItem(
											_fb.checkedRows, _fc.idField,
											row._parentId);
									if (prow && prow.checkState == "checked"
											&& _fc.cascadeCheck) {
										_ff = 1;
										row.checked = true;
										$.easyui.addArrayItem(_fb.checkedRows,
												_fc.idField, row);
									} else {
										if (row.checked) {
											$.easyui.addArrayItem(_fb.tmpIds,
													row[_fc.idField]);
										}
									}
									row.checkState = _ff
			...
			function _119(rows) {
				rows.sort(function(r1, r2) {
							...
							for (var i = 0; i < _117.length; i++) {
								var sn = _117[i];
								var so = _118[i];
								var col = $(_116).treegrid("getColumnOption",
										sn);
		...
		finder : {
			getTr : function(_126, id, type, _127) {
				...
				if (_127 == 0) {
					...
					if (type == "body") {
						var tr = $("#" + $.data(_126, "datagrid").rowIdPrefix
								+ "-" + _127 + "-" + id);
						if (!tr.length) {
							tr = (_127 == 1 ? dc.body1 : dc.body2)
									.find("tr[node-id=\"" + id + "\"]");
						}
						return tr;
					} else {
						if (type == "footer") {
							return (_127 == 1 ? dc.footer1 : dc.footer2)
									.find("tr[node-id=\"" + id + "\"]");
						} else {
							if (type == "selected") {
								return (_127 == 1 ? dc.body1 : dc.body2)
