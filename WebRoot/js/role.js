...
function deleteRole() {
	var row = $('#dg').datagrid('getSelected');
	if (row) {
		$.messager.confirm('确认', '您确定要删除编号为：' + row.roleId + '的角色吗?', function(
						r) {
					if (r) {
						$.post('roleDelete.action', {
									roleId : row.roleId
								});
						$('#dg').datagrid('reload');
					}
...
function openAuth() {
	...
	$('#authUnSelectedList').datalist({
				url : 'authUnSelectedList.action?roleId=' + roleId
			});
...
function leftToRight() {
	...
	for (var i = 0; i < unSelectedList.length; i++) {
		...
		if (flag == true) {
			...
			var rowIndex = $('#authUnSelectedList').datalist('getRowIndex',
					unSelectedList[i]);
...
function rightToLeft() {
	...
	for (var i = 0; i < selectedList.length; i++) {
		...
		var rowIndex = $('#authSelectedList').datalist('getRowIndex',
				selectedList[i]);
